...
fn get_visibility_for_shadow(world_position: vec3<f32>, texture_shadow_map: texture_depth_2d, sampler_shadow_map: sampler_comparison, light_as_camera: CameraUniform, normal: vec3<f32>, cascade_settings: CascadeSettingsUniform) -> f32 {
    ...
    let bias = max(cascade_settings.max_bias * (1.0 - dot(normal, -normalize(cascade_settings.light_dir))), cascade_settings.min_bias);
    for (var y = -1; y <= 1; y++) {
        for (var x = -1; x <= 1; x++) {
