...
fn compute_final_color(shadow_visibility: f32, world_position: vec3<f32>, camera_position: vec3<f32>, normal: vec3<f32>, albedo: vec4<f32>, emissive: vec4<f32>, ao: f32, roughness: f32, metallic: f32) -> vec3<f32> {
    ...
    // compute ambient using irradiance map
    let kS: vec3<f32> = fresnelSchlick(clamp(dot(N, V), 0.0, 1.0), F0);
    var kD: vec3<f32> = vec3<f32>(1.0, 1.0, 1.0) - kS;
    kD *= 1.0 - metallic;
    let irradiance: vec3<f32> = textureSample(irradiance_map, irradiance_map_sampler, N).rgb;
    let diffuse: vec3<f32> = irradiance * albedo.rgb;
    let ambient: vec3<f32> = (kD * diffuse) * ao;

    // combine colors from lights and ambient computed from IBL
